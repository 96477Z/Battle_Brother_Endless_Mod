this.el_special_effect_beast_entry <- this.inherit("scripts/skills/el_entrys/el_accessory_entry", {
	m = {
        EL_DamageMult = 0.0,
	...
	function getTooltip( _id )
	{
		...
		if(this.m.EL_CurrentLevel != 1)
		{
			return {
				id = _id,
				type = "text",
				text = "[color=" + colour + "]对野兽伤害 + " + this.Math.round(this.m.EL_CurrentLevel * this.m.EL_DamageMult * 100) * 0.01 + "%，减伤 + " + this.Math.round(this.m.EL_CurrentLevel * this.m.EL_ReduceDamageReceivedMult * 100) * 0.01 + "% (" + this.m.EL_DamageMult + "%)[/color]"
			};
		}
		else
		{
			return {
				id = _id,
				type = "text",
				text = "[color=" + colour + "]对野兽伤害 + " + this.m.EL_DamageMult + "%，减伤 + " + this.m.EL_ReduceDamageReceivedMult + "%[/color]"
			};
		}
	}
	...
	function EL_refreshTotalEntry( _EL_totalEntry )
	{
		...
		_EL_totalEntry.m.EL_DamageMultForBeast *= 1.0 + this.m.EL_CurrentLevel * this.m.EL_DamageMult * 0.01;
		_EL_totalEntry.m.EL_DamageReceivedMultForBeast /= 1.0 + this.m.EL_CurrentLevel * this.m.EL_ReduceDamageReceivedMult * 0.01;
	}
