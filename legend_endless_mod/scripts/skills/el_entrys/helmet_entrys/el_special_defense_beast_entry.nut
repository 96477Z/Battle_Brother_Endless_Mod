this.el_special_defense_beast_entry <- this.inherit("scripts/skills/el_entrys/el_accessory_entry", {
	...
	function create()
	{
		...
		this.m.ID = this.Const.EL_Helmet.EL_Entry.Factor.EL_SpecialDefenseBeast.ID;
	}
	...
	function getTooltip( _id )
	{
		...
		if(this.m.EL_CurrentLevel != 1)
		{
			return {
				id = _id,
				type = "text",
				text = "[color=" + colour + "]被野兽攻击时额外 + " + this.Math.ceil(this.m.EL_CurrentLevel * this.m.EL_Bonus) + "头部血量固定减伤和头部护甲固定减伤[/color]"
			};
		}
		else
		{
			return {
				id = _id,
				type = "text",
				text = "[color=" + colour + "]被野兽攻击时额外 + " + this.Math.ceil(this.m.EL_Bonus) + "头部血量固定减伤和头部护甲固定减伤[/color]"
			};
		}
	}	
