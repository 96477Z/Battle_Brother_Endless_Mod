this.el_round_swing_skill <- this.inherit("scripts/skills/skill", {
	m = {},
	function create()
	{
	...
	function onUse( _user, _targetTile )
	{
        ...
        local attack_time = 1;
        local rarity_entry = this.getContainer().getSkillByID("el_rarity_entry.breaking_through_a_thousand_armies")
        if(rarity_entry != null && rarity_entry.EL_isUsable())
        {
            attack_time += rarity_entry.m.EL_Stack;
        }
        for( attack_time; attack_time > 0; --attack_time)
        {
