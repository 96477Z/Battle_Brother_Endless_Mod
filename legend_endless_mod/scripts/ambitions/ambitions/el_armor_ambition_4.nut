this.el_armor_ambition_4 <- this.inherit("scripts/ambitions/ambition", {
	m = {
        EL_RankNeed = this.Const.EL_Item.Type.Legendary,
        EL_ConditionMaxNeed = 200
    },
	function create()
	{
		this.ambition.create();
		this.m.ID = "ambition.el_armor_4";
		...
		this.m.UIText = "拥有3件200+基础耐久的4阶盔甲和头盔";
		this.m.TooltipText = "拥有3件至少4阶的盔甲和头盔，每件装备都有200或以上的基础耐久。 Whether you buy them or loot them off of the battlefield, they will protect your men equally well.";
	...
	function getArmor()
	{
		...
		foreach( bro in roster )
		{
			...
			if (item != null)
			{
				if (item.EL_getBaseWithRankConditionMax() >= this.m.EL_ConditionMaxNeed && item.EL_getRankLevel() >= EL_RankNeed)
				{
					++ret.Helmet;
				}
			}
	...
	{
		if (!this.World.Ambitions.getAmbition("ambition.el_armor_3").isDone())
		{
			return;
		}
